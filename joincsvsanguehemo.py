...
for dirName, subdirList, fileList in os.walk(rootDir, topdown=False):
    ...
    for fname in fileList:
        ...
        try:
            ...
            var = var.replace(['-'], '0')
            if (count == 0):
                var.to_csv(finalDir+'\ok.csv',header=True, encoding='utf-8', sep=";" , columns = ['cod_municipio', 'desc_municipio','uf','regiao','Qtd.aprovada','Valor aprovado','Qtd.apresentada','Valor apresentado','Diferenca de valor','Valor do complemento federal','Valor do complemento do gestor','Valor do incremento', 'mes', 'ano', 'procedimento' ], index=False)
                #shutil.move(filepath, targetDir +"\"+fname)
            else:
                var.to_csv(finalDir+'\ok.csv', mode='a', header=False, encoding='utf-8', sep=";" , columns = ['cod_municipio', 'desc_municipio','uf','regiao','Qtd.aprovada','Valor aprovado','Qtd.apresentada','Valor apresentado','Diferenca de valor','Valor do complemento federal','Valor do complemento do gestor','Valor do incremento' , 'mes', 'ano', 'procedimento' ],  index=False)    
                #shutil.move(filepath, targetDir +'\\'+fname)        
            count +=1
            del var
